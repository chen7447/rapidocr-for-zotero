...
// Run: node --import tsx scripts/check-ort.ts
...
// Point ort at the wasm binary (Node needs a file:// URL, not a bare path)
const wasmPath = path.join(root, "node_modules", "onnxruntime-web", "dist", "ort-wasm-simd-threaded.jsep.wasm");
ort.env.wasm.wasmPaths = { wasm: "file:///" + wasmPath.replace(/\\/g, "/") };
...
async function main() {
  ...
  for (const name of det.outputNames) {
    ...
    console.log(`  prob map min=${mn}, max=${mx}, mean=${(sum / data.length).toFixed(6)}, nonZero=${nz} (${((nz / data.length) * 100).toFixed(3)}%)`);

    // Also print some sample values
    const sample = Array.from(data.slice(0, 20)).map((v) => v.toFixed(6)).join(", ");
    console.log(`  first 20 values: ${sample}`);
  }
}
